...
## run_service で GitHub まで到達しない時の2分診断

`run_service` 実行後に GitHub Issue 作成まで進まない場合、以下を順に確認する。

1. `service` 値
- `run_service` 入力の `service` が、期待する統合キー（例: `github`）と一致しているか。

2. `campaign.target_tools`
- 対象キャンペーンの `target_tools` に、上記 `service` が含まれているか。

3. `sponsored_apis.service_key`
- バックエンドの `sponsored_apis` レコードで、`service_key` が同じ値になっているか。

4. `run_service` 応答の `output`
- `output` が空文字/期待外フォーマットの場合は upstream 側応答をログで確認する。

### 切り分けテンプレ（そのまま貼り付け）

```text
run_service の切り分けをします。以下を順番に報告してください。
1) 実行した service 値
